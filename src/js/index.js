...
$(function () {
    ...
    $('.to').each(function () {
		$(this).css({
			width: $(this).parent().parent().parent().children('.down').children('.right').children('span').text() + '%'
		})
	})
	...
	let kill_round = Math.round(Math.random() * 4 + 1);
	for (let i = 1; i <= 4; i++) {
		if (kill_round === 1) {
			$('.time .round').text('08:00场 ');
			kill(08);
			break;
		} else if (kill_round === 2) {
			$('.time .round').text('12:00场 ');
			kill(12);
			break;
		} else if (kill_round === 3) {
			$('.time .round').text('18:00场 ');
			kill(18);
			break;
		} else if (kill_round === 4) {
			$('.time .round').text('23:00场 ');
			kill(23);
			break;
		}
	}
	...
	function kill(num) {
		let time = setInterval(() => {
			...
			let str = year + '/' + month + '/' + day + ' ' + num + ':00:00';

			let end_date = new Date(str);
			let now = date.getTime();
			let end = end_date.getTime();

			let i = Math.round(end - now);
			let h = Math.floor(i / 1000 / 60 / 60 % 24);
			let m = Math.floor(i / 1000 / 60 % 60);
